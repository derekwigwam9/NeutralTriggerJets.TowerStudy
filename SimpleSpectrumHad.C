...
void SimpleSpectrumHad(const Bool_t isInBatchMode=false, const TString sInput=sInDefault, const TString sOutput=sOutDefault) {
  ...
  TH1D *hTrgPt  = new TH1D("hTrgPt", "", nPt, pT[0], pT[1]);
  // track histograms
  ...
  for (UInt_t iEvt = 0; iEvt < nEvts; iEvt++) {
    ...
    // track loop 1
    for (UInt_t iTrk1 = 0; iTrk1 < nTrks; iTrk1++) {

      const Double_t nFitTrg = PrimaryTrackArray_nHitsFit[iTrk1];
      const Double_t nPosTrg = PrimaryTrackArray_nHitsPoss[iTrk1];
      const Double_t rFitTrg = nFitTrg / nPosTrg;
      const Double_t dcaTrg  = PrimaryTrackArray_dcag[iTrk1];
      const Double_t hTrg    = PrimaryTrackArray_eta[iTrk1];
      const Double_t fTrg    = PrimaryTrackArray_phi[iTrk1];
      const Double_t pTtrg   = PrimaryTrackArray_pT[iTrk1];
      const Double_t nSigPi  = PrimaryTrackArray_nSigPion[iTrk1];
      const Double_t nSigK   = PrimaryTrackArray_nSigKaon[iTrk1];
      const Double_t nSigP   = PrimaryTrackArray_nSigProton[iTrk1];
      const Double_t nSigE   = PrimaryTrackArray_nSigElectron[iTrk1];

      // pid
      const Bool_t isPion     = (TMath::Abs(nSigPi) < nSigCut);
      const Bool_t isKaon     = (TMath::Abs(nSigK) < nSigCut);
      const Bool_t isProton   = (TMath::Abs(nSigP) < nSigCut);
      const Bool_t isElectron = (TMath::Abs(nSigE) < nSigCut);
      const Bool_t isHadron   = ((isPion || isKaon || isProton) && !isElectron);


      // trigger cuts
      const Bool_t isInFitTrgCut   = (nFitTrg > nFitMin);
      const Bool_t isInRatioTrgCut = (rFitTrg > rFitMin);
      const Bool_t isInDcaTrgCut   = (dcaTrg < dcaMax);
      const Bool_t isInEtaTrgCut   = (TMath::Abs(hTrg) < hTrgMax);
      const Bool_t isInPtTrgCut    = ((pTtrg > eTtrgMin) && (pTtrg < eTtrgMax));
      const Bool_t isGoodTrg       = (isInFitTrgCut && isInRatioTrgCut && isInDcaTrgCut && isInEtaTrgCut && isInPtTrgCut && isHadron);
      if (isGoodTrg) {

        // track loop 2
        for (UInt_t iTrk2 = 0; iTrk2 < nTrks; iTrk2++) {

          // exclude trigger
          if (iTrk2 == iTrk1) continue;

          const Double_t nFitTrk = PrimaryTrackArray_nHitsFit[iTrk2];
          const Double_t nPosTrk = PrimaryTrackArray_nHitsPoss[iTrk2];
          const Double_t rFitTrk = nFitTrk / nPosTrk;
          const Double_t dcaTrk  = PrimaryTrackArray_dcag[iTrk2];
          const Double_t hTrk    = PrimaryTrackArray_eta[iTrk2];
          const Double_t fTrk    = PrimaryTrackArray_phi[iTrk2];
          const Double_t pTtrk   = PrimaryTrackArray_pT[iTrk2];

          Double_t dHtrk = hTrk - hTrg;
          Double_t dFtrk = fTrk - fTrg;
          if (dFtrk < (-1. * TMath::PiOver2())) dFtrk += TMath::TwoPi();
          if (dFtrk > (3. * TMath::PiOver2()))  dFtrk -= TMath::TwoPi();

          // track cuts
          const Bool_t isInFitTrkCut   = (nFitTrk > nFitMin);
          const Bool_t isInRatioTrkCut = (rFitTrk > rFitMin);
          const Bool_t isInDcaTrkCut   = (dcaTrk < dcaMax);

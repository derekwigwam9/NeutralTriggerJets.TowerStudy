...
void DataTowerStudy_hot(const Bool_t isInBatchMode=false, const TString sInput=sInDefault, const TString sOutput=sOutDefault) {
  ...
  hClustVsTrgEta = new TH2D("hClustVsTrgEta", "Trigger tower vs. cluster #eta;#eta^{twr};#eta^{clust}", nH, h[0], h[1], nH[0], h[0], h[1]);
  hVtxZ          -> Sumw2();
  hVtxR          -> Sumw2();
  hTrgEta        -> Sumw2();
  hClustEta      -> Sumw2();
  ...
  for (Long64_t iEvt = 0; iEvt < nEvts; iEvt++) {
    ...
    if (isInAdcCut && isInStrpCut && isInProjCut && isInEtaTrgCut && isInEneTrgCut && isInTspTrgCut) {
      hTrgEta        -> Fill(hTrg);
      hClustEta      -> Fill(hClust);
      hTrgPhi        -> Fill(fClust);
      hTrgEt         -> Fill(eTtrg);
      hTrgPhiVsEta   -> Fill(hClust, fClust);
      hClustVsTrgEta -> Fill(hTrg, hClust);
    }
    hVtxZ -> Fill(zVtx);
    hVtxR -> Fill(rVtx);

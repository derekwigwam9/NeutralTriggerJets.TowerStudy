...
static const UInt_t nHotTwrs(86);
...
static const TString sInDefault("TowerResponseStudy/Run9/pp200r9.epCalc2.root");
static const TString sOutDefault("pp200r9.highTwr.d8m9y2017.root");
static const TString sHotTwrList("lists/HotTowerCheck.pp200r9.d14m8y2017.list");
static const TString sTrkDirs[nDist] = {"AllTracks", "RecoilTracks"};
static const TString sTwrDirs[nDist] = {"AllTowers", "RecoilTowers"};



void DataTowerStudy(const Bool_t isInBatchMode=false, const TString sInput=sInDefault, const TString sOutput=sOutDefault) {
  ...
  hTrgTsp           = new TH1D("hTrgTsp", "Trigger TSP", nS, s1, s2);
  // E/p histograms
  ...
  for (Long64_t iEvt = 0; iEvt < nEvts; iEvt++) {
    ...
    if (isInTrgMode1) {
      ...
      const Bool_t foundTrg1 = (isInEtaTrgCut && isInEneTrgCut && isInTspTrgCut && !isHotTrg);
      if (!foundTrg1)
        continue;
      else {
        hTrgEt  -> Fill(eTtrg);
    ...
    if (isInTrgMode3) {
      ...
      for (Long64_t iTwr = 0; iTwr <  nTwrs; iTwr++) {
        ...
        if (!isTriggerTwr && isInEtaTwrCut && isInEneTwrCut && hasOneMatch) {
          hEpTwrEne -> Fill(eTwr);

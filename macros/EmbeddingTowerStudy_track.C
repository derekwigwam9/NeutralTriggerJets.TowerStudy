...
static const UInt_t fTrgMode(2);
// io parameters
static const TString sTree("GfmtoDst_mu");
static const TString sInDefault("/star/data01/pwg/dmawxc/JetReco_pp/FullJetTree/run9embedding/pt9.match.root");
static const TString sOutDefault("pp200r9pt9u.et9vz55track.root");
...
void EmbeddingTowerStudy_track(const Bool_t isInBatchMode=false, const TString sInput=sInDefault, const TString sOutput=sOutDefault) {
  ...
  for (Long64_t iEvt = 0; iEvt < nEvts; iEvt++) {
    ...
    if (isInTrgMode2) {
      ...
      for (Long64_t iTrk = 0; iTrk < nTrks; iTrk++) {
        ...
        const Bool_t isInEtTrgCut   = ((pTtrk > eTtrgMin) && (pTtrk < eTtrgMax));
        if (!isInEtaTrgCut || !isInEtTrgCut) continue;

        // check if trigger
        if (isInEtTrgCut && !isElectron) {
          iTrig     = iTrk;
          fTrig     = fTrk;
          hTrig     = hTrk;
          foundTrg2 = true;
    ...
    if (isInTrgMode3) {
      ...
      for (Long64_t iTwr = 0; iTwr <  nTwrs; iTwr++) {
        ...
        const Double_t pZtwr  = TowerArray_TwrPz[iTwr];
        ...
        if (removeHotTwrs) {
          for (Long64_t iHot = 0; iHot < nHotTwrs; iHot++) {
            if (twrID == hotTwrs[iHot]) {
              isHot = true;
              hHotTrgPhiVsEta -> Fill(hTwr, fTwr);
              break;
            }
          }
          if (isHot) continue;
    ...
    for (Long64_t iTrk = 0; iTrk < nTrks; iTrk++) {
      ...
      // exclude trigger
      if (isInTrgMode2) {
        const Bool_t trackIsTrigger = (iTrk == iTrig);
        if (trackIsTrigger) continue;
      }
    ...
    for (Long64_t iTwr = 0; iTwr <  nTwrs; iTwr++) {
      ...
      // exclude trigger
      if (isInTrgMode3) {
        const Bool_t towerIsTrigger = (twrID == iTrig);
        if (towerIsTrigger) continue;
      }
